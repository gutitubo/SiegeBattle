...
public class RuneInventory {
	...
	public static Inventory getRuneInventory (Player p) {
		RuneCategory main = null;
		RuneCategory sub = null;
		main = Siege.SiegeBattleMain.preSelect.get(p).getMain();
		sub = Siege.SiegeBattleMain.preSelect.get(p).getSub();
		if (main == null) {
			return getRunePathInventory(0, p);
		} else if (sub == null) {
			return getRunePathInventory(1, p);
		}
		Inventory inv = Bukkit.createInventory(null, 54, RUNE_INV_TITLE_MAIN);
		int i = 0; //index
		/* MainのTier1ルーン */
		for (Runes r : Runes.asHashSet(main, 1)) {
			inv.setItem(i, r.toItemStack());
			if (isPreHolder(r, p)) {
				ItemStack selected = r.toItemStack();
				ItemFactory.setSelected(selected);
		...
		/* MainのTier2ルーン */
		i = 9;
		for (Runes r : Runes.asHashSet(main, 2)) {
			inv.setItem(i, r.toItemStack());
			if (isPreHolder(r, p)) {
				ItemStack selected = r.toItemStack();
				ItemFactory.setSelected(selected);
		...
		/* SubのTier2ルーン */
		i = 27;
		for (Runes r : Runes.asHashSet(sub, 2)) {
			inv.setItem(i, r.toItemStack());
			if (isPreHolder(r, p)) {
				ItemStack selected = r.toItemStack();
				ItemFactory.setSelected(selected);
	...
	public static Inventory getRunePathInventory(int state, Player p) { //state 0 = main nothing 1 = subnothing
		String str ;
		str = state == 0 ? RUNE_INV_TITLE_PATH : RUNE_INV_TITLE_PATH_SUB;
		Inventory inv = Bukkit.createInventory(null, 9, str);
		inv.setItem(2, ItemFactory.getRunePathIcon(RuneCategory.ATTACK, false)); //攻撃
		...
		if (str.equalsIgnoreCase(RUNE_INV_TITLE_PATH_SUB)) { //サブの場合
			int main = 0;
			switch(Siege.SiegeBattleMain.preSelect.get(p).getMain()) {
			case ATTACK:
				main = 2;
				break;
			case MAGIC:
	...
	public static boolean isPreHolder(Runes rune, Player p) {
		Runes[] currentRunes = Siege.SiegeBattleMain.preSelect.get(p).getRunes();
